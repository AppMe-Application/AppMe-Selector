...
public class IconPreview {
    ...
    public IconPreview(Activity activity) {
        mContext = activity;
        mWidth = (int) mContext.getResources().getDimension(R.dimen.apps_item_height);
    ...
    private static void loadFromRes(final File file, final ImageView icon) {
        ...
        if (file != null && file.isDirectory()) {
            ...
            if (file.canRead() && files != null && files.length > 0)
                mimeIcon = mResources.getDrawable(R.drawable.ic_folder);
            else
                mimeIcon = mResources.getDrawable(R.drawable.ic_folder_open);
        ...
        if (mimeIcon != null) {
            ...
            // default icon
            icon.setImageResource(R.drawable.ic_file_question);
        }
    }
