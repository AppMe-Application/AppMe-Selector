...
public class FilePickerPreference extends Preference implements DialogSelectionListener, Preference.OnPreferenceClickListener {
    ...
    public void onSelectedFilePaths(String[] files) {
        ...
        for (String path:files) {   buff.append(path).append(":");
        }
    ...
    private void initProperties(Context context, AttributeSet attrs) {	
        ...
        TypedArray tarr = context.getTheme().obtainStyledAttributes(attrs, R.styleable.FilePickerPreference, 0, 0);
        final int N = tarr.getIndexCount();
        for (int i = 0; i < N; ++i) {   int attr = tarr.getIndex(i);
